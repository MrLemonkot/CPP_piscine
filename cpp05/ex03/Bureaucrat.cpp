...
void	Bureaucrat::executeForm(Form const & form)
{
	...
	rez = form.execute(*this);
	if (rez == -2)
		std::cerr << this->_name << " couldn’t execute " << form.getName() << " because that form is not signed." << std::endl;
	else if (rez == -1)
		std::cerr << this->_name << " couldn’t execute " << form.getName() << " because his grade is too low." << std::endl;
	else
